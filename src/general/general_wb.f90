module mod_general_wb
  ...
  private
  public :: f_general_wb, f_d_general_wb, f_z_general_wb, &
       f_general_to_wb, f_d_general_to_wb, f_z_general_to_wb, &
       general_to_wb, d_general_to_wb, z_general_to_wb, &
       d_wb_of_general, z_wb_of_general, wb_of_general, wb

  interface wb_of_general
     module procedure d_wb_of_general, z_wb_of_general
  ...
  interface f_general_wb
     module procedure f_d_general_wb, f_z_general_wb
  end interface f_general_wb

  interface f_general_to_wb
     module procedure f_d_general_to_wb, f_z_general_to_wb
  end interface f_general_to_wb

  interface general_to_wb
     module procedure d_general_to_wb, z_general_to_wb
...
contains
  ...
  function z_wb_of_general(a, ubw, lbwmax, ubwmax, tol, error) result(wb)
    type(z_wb), allocatable :: wb
    complex(kind=dp), target, dimension(:,:), intent(inout) :: a
    type(error_info), intent(inout), optional :: error
    real(kind=dp), intent(in) :: tol
    integer(kind=int32), intent(in) :: ubw, lbwmax, ubwmax
    type(routine_info), parameter :: info=info_z_wb_of_general
    integer(kind=int32) :: n
    ...
    call z_general_to_wb(a,wb,ubw,tol,error)

    call pop_id(error)
    
  end function z_wb_of_general
  

  subroutine f_z_general_wb(a, n, lbws, lbwmax, numrotsw, jsw, csw, ssw, tol, error)
    ...
    type(routine_info), parameter :: info=info_f_z_general_wb

    if (failure(error)) return
    call push_id(info, error)
    
    call ip_transpose(a)
    call f_z_general_bv(a,n,lbws,lbwmax,numrotsw,ksv,csv,ssv,tol,error)
    ...
    call pop_id(error)
  end subroutine f_z_general_wb

  ! Errors:
  ! 0: no error
  ! 1: insufficient lower bw in wb%bc
  subroutine f_z_general_to_wb(a, n, b, lbw, ubw, lbwmax, ubwmax, &
       numrotsw, jsw, csw, ssw, tol, error)
    complex(kind=dp), target, dimension(n,n), intent(inout) :: a
    integer(kind=int32), dimension(lbwmax,n), intent(out) :: jsw
    real(kind=dp), dimension(lbwmax,n), intent(out) :: csw
    ...
    !
    integer(kind=int32), dimension(n) :: lbws
    type(routine_info), parameter :: info=info_f_z_general_to_wb
    ...
    call f_z_general_wb(a, n, lbws, lbwmax, numrotsw, jsw, csw, ssw, tol, error)

    if (success(error)) then
       
       lbw=maxval(lbws)
       if (maxval(lbws) > lbwmax) then
          ! This should already have been detected in f_z_general_wb.
          call set_error(1, info, error); return
       else
    ...
    call pop_id(error)
  end subroutine f_z_general_to_wb
  ...
  ! 2: wb%ubwmax < ubw
  ! 3: n is not the same for a and wb.
  subroutine z_general_to_wb(a,wb,ubw,tol,error)
    complex(kind=dp), target, dimension(:,:), intent(inout) :: a
    type(z_wb), intent(inout) :: wb
    type(error_info), intent(inout), optional :: error
    real(kind=dp), intent(in) :: tol
    integer(kind=int32), intent(in) :: ubw
    type(routine_info), parameter :: info=info_z_general_to_wb
    ...
    call f_z_general_to_wb(a,get_n(wb),wb%bc, wb%lbw, ubw, get_lbwmax(wb), get_ubwmax(wb), &
         wb%numrotsw, wb%jsw, wb%csw, wb%ssw, tol, error)
    wb%ubw=ubw
    call pop_id(error)
  end subroutine z_general_to_wb
