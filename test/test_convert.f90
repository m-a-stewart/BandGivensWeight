program test_convert
  ...
  a0=a
  call f_upper_to_ub(a,n, b_ub, lbw, ubw, lbwmax, ubwmax, numrots_ub, j1s, j2s,  &
       cs_ub, ss_ub, tol, error)
  if (error == 1) then
     print *, "orthogonalization error in real UB"
  else
     call cpu_time(t1)
     call f_ub_to_bv(b_ub, n, lbw, ubw, lbwmax, ubwmax, numrots_ub, j1s, j2s, cs_ub, ss_ub, &
          b_bv, lbwmax, ubwmax, numrots_bv, k1s, k2s, cs_bv, ss_bv, error)
     call cpu_time(t2)
     if (error > 0) then
  ...
  ! bv to ub
  a=a0
  call f_upper_to_bv(a,n, b_bv, lbw, ubw, lbwmax, ubwmax, numrots_bv, k1s, k2s,  &
       cs_bv, ss_bv, tol, error)
  if (error == 1) then
     print *, "orthogonalization error in real BV"
  else
     call cpu_time(t1)
     call f_bv_to_ub(b_bv, n, lbw, ubw, lbwmax, ubwmax, numrots_bv, k1s, k2s, cs_bv, ss_bv, &
          b_ub, lbwmax, ubwmax, numrots_ub, j1s, j2s, cs_ub, ss_ub, error)
     call cpu_time(t2)
     if (error > 0) then
...
! complex
  ...
  ! ub to bv
  a0_c=a_c
  call f_upper_to_ub(a_c,n, b_ub_c, lbw, ubw_c, lbwmax, ubwmax, numrots_ub_c, j1s_c, j2s_c,  &
       cs_ub_c, ss_ub_c, tol, error)
  if (error == 1) then
     print *, "orthogonalization error in complex UB"
  else
     call cpu_time(t1)
     call f_ub_to_bv(b_ub_c, n, lbw, ubw_c, lbwmax, ubwmax, numrots_ub_c, j1s_c, j2s_c, cs_ub_c, ss_ub_c, &
          b_bv_c, lbwmax, ubwmax, numrots_bv_c, k1s_c, k2s_c, cs_bv_c, ss_bv_c, error)
     call cpu_time(t2)
     if (error > 0) then
        print *, "Error in upper_ub_to_bv:", error
     else
  ...
  ! bv to ub
  a_c=a0_c
  call f_upper_to_bv(a_c,n, b_bv_c, lbw, ubw_c, lbwmax, ubwmax, numrots_bv_c, k1s_c, k2s_c,  &
       cs_bv_c, ss_bv_c, tol, error)
  if (error == 1) then
     print *, "orthogonalization error in complex BV"
  else
     call cpu_time(t1)
     call f_bv_to_ub(b_bv_c, n, lbw, ubw_c, lbwmax, ubwmax, numrots_bv_c, k1s_c, k2s_c, cs_bv_c, ss_bv_c, &
          b_ub_c, lbwmax, ubwmax, numrots_ub_c, j1s_c, j2s_c, cs_ub_c, ss_ub_c, error)
     call cpu_time(t2)
     if (error > 0) then
        print *, "Error in upper_bv_to_ub:", error
     else
